...
|---|---|---|
| `DATABASE_URL` | `sqlite:///./storage/talentrank.db` | Any SQLAlchemy URL. Postgres driver (`psycopg2-binary`) is bundled — just set `postgresql://user:pass@host/dbname`. For MySQL: `pip install PyMySQL` then `mysql+pymysql://user:pass@localhost:3306/resume_db` |
...
ephemeral disk — SQLite data resets on redeploy, wiping every registered
account. For persistent data, point `DATABASE_URL` at a free hosted Postgres
(e.g. [Neon](https://neon.tech) or [Supabase](https://supabase.com)) instead —
the Postgres driver is already bundled, so setting the env var on Render and
restarting is all that's needed.
